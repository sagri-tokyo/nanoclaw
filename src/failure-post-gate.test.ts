...
describe('decideOutcomeDisposition', () => {
  ...
  function postedAt(recordedAt: string) {
    commitTaskOutcome(
  ...
  it('reports a repeat without consulting the threshold', () => {
    const scheduled = task(2);
    postedAt('2026-05-15T00:15:00.000Z');
  ...
  it('still calls a record posted inside the previous run a repeat', () => {
    // Runs at 01:00 and 02:00 put `previousRunCutoff` (OFFSET 1, the end of
    // run N-2) at 01:00. A record stamped after it belongs to the run that
    // reported it and must stay suppressed. Stamping it before every run would
    // pass under OFFSET 0 too and pin nothing.
    const scheduled = task(2);
    postedAt('2026-05-15T01:30:00.000Z');
  ...
  it('treats a record posted before the previous run as news again', () => {
    // Same cutoff of 01:00, stamp moved to the far side of it: an older stamp
    // is a later outage, not a repeat, which is what stops dedupe becoming
    // permanent silence.
    const scheduled = task(2);
    postedAt('2026-05-15T00:30:00.000Z');
    priorRun(scheduled.id, 'error', '2026-05-15T01:00:00.000Z');
    priorRun(scheduled.id, 'error', '2026-05-15T02:00:00.000Z');
    expect(decideOutcomeDisposition(scheduled, failure)).toBe('posted');
  });
