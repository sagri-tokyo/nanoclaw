...
One documented exception: when telemetry is enabled, the OTLP collector auth header (`OTEL_EXPORTER_OTLP_HEADERS`) is forwarded into the container directly so Claude Code's OTel exporter can authenticate to the collector. See [docs/SECURITY.md](docs/SECURITY.md) §7 for why this credential is scoped and accepted.
