...
| `sub_request` / `fetch_untrusted_list` | `src/reader-rpc.ts` | Per-request UUID | Same ok/error/hashing semantics as `fetch_untrusted`. `trigger_source` and `group` carry the `source_type` param (`arxiv_search`, `github_search`, `github_pr_list`, `github_issue_list`, `github_run_list`, `notion_database_query`). |
