...
| `sub_request` / `fetch_untrusted` | `src/reader-rpc.ts` | Per-request UUID | One per `fetch_untrusted` call. Success (`outcome=ok`) and failure (`outcome=error`, `error_class` populated from the thrown error's constructor name) both emit. `trigger_source` and `group` carry the `source_type` param (`web_content`, `github_issue`, `github_comment`, `notion_page`). `inputs_hash` covers the full RPC `params` object; `outputs_hash` is the laundered `ReaderOutput` on success or a `hashFailureOutput({error_class})` digest on failure. |
| `sub_request` / `fetch_untrusted_list` | `src/reader-rpc.ts` | Per-request UUID | Same ok/error/hashing semantics as `fetch_untrusted`. `trigger_source` and `group` carry the `source_type` param (`arxiv_search`, `github_search`, `github_pr_list`, `github_issue_list`, `github_run_list`, `notion_database_query`, `notion_search`). |
