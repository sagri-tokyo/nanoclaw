...
**Reader RPC (container-side access).** `src/reader-rpc.ts` exposes the reader primitive as an HTTP endpoint so skills running inside the container can launder their own fetches. `POST /rpc` with `{"method": "read_untrusted", "params": {raw, source, source_metadata}}` → 200 `ReaderOutput`. Bound to the address returned by `getProxyBindHost()` (loopback on macOS/WSL, docker0 on Linux); reachable from containers via `host.docker.internal` and not from LAN. When `docker0` is absent (Podman, custom bridge networks, unusual hosts) and `CREDENTIAL_PROXY_HOST` is not set, nanoclaw refuses to start rather than falling back to `0.0.0.0`. An explicit non-loopback `CREDENTIAL_PROXY_HOST` is permitted (operator responsibility) but logged as a warning; the bound address is also logged on startup. Containers receive `NANOCLAW_READER_RPC_URL` in their environment. Trust model: containers are semi-trusted — they can burn reader API calls (cost DoS bounded by Anthropic rate limits) but cannot extract host credentials; the reader makes its own outbound Anthropic call and never echoes env vars into its output. Reader errors surface as HTTP 5xx with a static message body (never the upstream Anthropic response text, which could itself echo the untrusted input). Request body is capped at 256 KiB. `src/reader-rpc.test.ts` covers schema validation, unknown methods, size limits, and asserts the injection payload is absent from the response even when the reader upstream body contains it verbatim.
...
- **Bounded to the collector, and full-privilege there.** A leaked value reaches the telemetry collector and nothing else: not API keys, not channel auth, not the OneCLI vault, not any application data store. On the collector it is not ingest-only. Langfuse authenticates the OTLP endpoint with the project's `pk:sk` pair over Basic auth, the same pair authorizes the public API, and both route through one project-level auth wrapper, so a leaked header reads, forges, and deletes every trace in the project. Scoped keys are an open Langfuse request ([#7104](https://github.com/orgs/langfuse/discussions/7104), open since 2025-06; a read-only PR closed unmerged 2026-04; checked 2026-07-16), so no provisioning narrows this today. Those traces are not neutral: `buildTelemetryEnv` stamps `enduser.id` (the triggering Slack user) and `tenant.id` (the group/task) onto every span, so they record who ran what, when. What makes that acceptable is not that the data is uninteresting, but that the operator enabling self-hosted telemetry is choosing to put exactly this data in exactly this collector; the header grants an attacker the access the operator already granted the collector, and nothing beyond it.

**Residual risk (accepted knowingly):** unlike OneCLI-injected credentials, this value *is* present in the container environment and on the host `docker run` argv — visible in `ps` and `docker inspect`, the same exposure `src/env-forward.ts` warns about. Since it cannot be scoped down, the exposure is the project's whole trace history, and deletion is the sharp end of it: irreversible, and not something rotation undoes after the fact. Rotate independently of OneCLI-managed secrets.

Note which attacker the network buys you anything against. A prompt-injected agent inside the container holds the header *and* a route to the collector by construction — that is what the forward exists for — so keeping the collector off the public internet does not bind it. That control bounds the host-argv reader who has the value but not the position, and is worth having for that reason alone. Against the in-container case the mitigations are rotation, and not sending the collector anything you would not accept losing.
