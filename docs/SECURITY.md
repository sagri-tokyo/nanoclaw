...
- **Bounded to the collector, but not low-privilege there.** A leaked value grants no access to API keys, user data, channel auth, or the OneCLI vault: it authenticates to the telemetry collector and nothing else. It is not, however, ingest-only. Langfuse authenticates the OTLP endpoint with the project's `pk:sk` pair over Basic auth, and the same pair authorizes the public API, so a leaked header reads every trace in the project as well as writing and deleting them. Scoped keys are an open Langfuse request ([#7104](https://github.com/orgs/langfuse/discussions/7104), open since 2025-06; a read-only PR was closed unmerged in 2026-04), so there is no provisioning that narrows this today. Treat the collector credential as full-privilege on its project, and size the exposure by what the traces themselves contain.

**Residual risk (accepted knowingly):** unlike OneCLI-injected credentials, this value *is* present in the container environment and on the host `docker run` argv — visible in `ps` and `docker inspect`, the same exposure `src/env-forward.ts` warns about. Since it cannot be scoped down, rotate it independently of OneCLI-managed secrets, keep the collector off the public internet so a leaked key needs network position to spend, and treat the traces as the thing being protected: what an attacker reads with this key is whatever the exporter has already sent.
