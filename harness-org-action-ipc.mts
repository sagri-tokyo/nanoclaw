...
function dropOrgAction(blob: Record<string, unknown>) {
  return processTaskIpc({ type: 'org_action', ...blob } as never, GROUP, true, ipcDeps);
}
function inbound(content: string, over: Partial<NewMessage> = {}) {
  handleInboundMessage(
    CHAT,
    {
      id: 'm', chat_jid: CHAT, sender: APPROVER, sender_name: 'Approver',
      content, timestamp: '2026-06-29T00:01:00.000Z',
      is_from_me: false, is_bot_message: false, ...over,
    },
    inboundDeps,
  );
}
...
async function main() {
  ...
  banner('A. IPC VALIDATION — malformed drops are rejected at the entry, gate never runs');
  console.log('  A1. missing target_ref:');
  await dropOrgAction({ action: 'notion.append_progress', reversibility: 'reversible', stakes_hint: 'safe', canonical_args: {} });
  console.log('  A2. stakes_hint outside the enum ("maybe"):');
  await dropOrgAction({ action: 'notion.append_progress', target_ref: 'a'.repeat(32), reversibility: 'reversible', stakes_hint: 'maybe', canonical_args: {} });
  console.log('  A3. canonical_args is an array, not an object (would-be coerced-to-{} attack):');
  await dropOrgAction({ action: 'notion.append_progress', target_ref: 'a'.repeat(32), reversibility: 'reversible', stakes_hint: 'safe', canonical_args: [] });
  console.log('  A4. citation_refs not a string array:');
  await dropOrgAction({ action: 'notion.append_progress', target_ref: 'a'.repeat(32), reversibility: 'reversible', stakes_hint: 'safe', canonical_args: {}, citation_refs: [1, 2] });
  console.log(`  -> gate executions so far: ${executed.length} (want 0)`);

  banner('B. SAFE drop through IPC -> classified execute -> write fires, audit ok');
  await dropOrgAction({ action: 'notion.append_progress', target_ref: 'a'.repeat(32), reversibility: 'reversible', stakes_hint: 'safe', canonical_args: { text: 'progress' } });

  banner('C. GATED drop through IPC -> held + Slack prompt (agent under-claims stakes_hint:safe)');
  mintNext = TOK('IPC1');
  await dropOrgAction({ action: 'notion.write_property', target_ref: 'a'.repeat(32), reversibility: 'reversible', stakes_hint: 'safe', canonical_args: { property: 'Status', value: 'Approved' } });
  console.log(`  db state: ${getPendingAction(mintNext)?.state}`);

  banner('D. INBOUND approve via handleInboundMessage -> intercepted, single execute');
  inbound(`approve ${TOK('IPC1')}`);
  await flush();
  console.log(`  db state: ${getPendingAction(TOK('IPC1'))?.state}, stored-as-ordinary: ${stored.length} (want 0)`);

  banner('E. INBOUND exactly-once — replay the same approve, no second execute');
  ...
  mintNext = TOK('IPC2');
  await dropOrgAction({ action: 'notion.write_property', target_ref: 'a'.repeat(32), reversibility: 'reversible', stakes_hint: 'gated', canonical_args: { property: 'Status', value: 'Ready for AI' } });
  ...
  await flush();
  console.log(`  executes from attacks: ${executed.length - exF} (want 0), row: ${getPendingAction(TOK('IPC2'))?.state}`);
  console.log('  F3. real approver:');
  inbound(`approve ${TOK('IPC2')}`);
  await flush();
  console.log(`  row: ${getPendingAction(TOK('IPC2'))?.state}`);

  banner('G. ADVERSARIAL stakes_hint via IPC — agent lies safe on a red-line target -> refused');
  const exG = executed.length;
  await dropOrgAction({ action: 'notion.append_progress', target_ref: 'a'.repeat(32), reversibility: 'reversible', stakes_hint: 'safe', canonical_args: { text: 'touch the mrv carbon ledger' } });
  console.log(`  writes: ${executed.length - exG} (want 0)`);

  banner('H. ORDINARY message falls through to storeMessage (not an approval keyword)');
  ...
  console.log(`  audit rows: ${audit.length} (${audit.map((a) => a.outcome).join(', ')})`);
}
